...
static void printUsage(const char* progName) {
    std::cerr << "Usage: " << progName
              << " <host> [port] [username] [password]" << std::endl;
    std::cerr << std::endl;
    std::cerr << "  host       Hostname or IP of the ITSCAM webapp backend"
              << std::endl;
    std::cerr << "  port       HTTP port (default: 80)" << std::endl;
...
int main(int argc, char* argv[]) {
    ...
    const uint16_t    port     = (argc >= 3) ? static_cast<uint16_t>(
                                                   std::stoi(argv[2])) : 80;
    const std::string username = (argc >= 4) ? argv[3] : "admin";
    const std::string password = (argc >= 5) ? argv[4] : "1234";

    log("ITSCAM REST Client SDK Example");
    log("Target: " + host + ":" + std::to_string(port));
