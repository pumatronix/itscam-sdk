...
//=========================================================================
// CGI snapshot request  (used by ItscamCgiClient::getSnapshot)
//=========================================================================

/// Parameters for /api/snapshot.cgi.  Mirrors the URL arguments parsed
/// server-side by SnapshotCGI::doCGI (camera-daemon/src/plugin/cgi/snapshot.h):
///
///    s=<csv>       per-exposure shutter (absolute or 0 = camera default)
///    g=<csv>       per-exposure gain    (absolute or 0 = camera default)
///    q=<int>       JPEG quality (0..100); -1 = use camera default
///    m=1           combine multi-exposure frames into a single mosaic
///    fmt=png       force PNG output (only valid for single exposure)
///    c=<int>       scenario id
///    r=<x0,y0,x1,y1>  user crop
///    t=<text>      text overlay ("tarja")
///    User_<key>=<value>  arbitrary user metadata baked into image tags
struct SnapshotCgiRequest {
    /// Per-exposure shutter values.  Empty -> use the camera's currently
    /// configured exposure list (most common).  When set, must have the
    /// same length as @ref gains.
    std::vector<int> shutters;

    /// Per-exposure gain values.  See @ref shutters.
    std::vector<int> gains;

    /// JPEG quality (0-100).  -1 selects the camera's DEFAULT_JPEG_QUALITY.
    int quality = -1;

    /// Combine multi-exposure responses into a single JPEG mosaic.
    bool mosaic = false;

    /// Image format.  Empty -> JPEG.  "png" forces PNG (single exposure only).
    std::string format;

    /// Scenario id (corresponds to the camera's active scenario set).
    /// -1 leaves the active scenario unchanged.
    int scenario = -1;

    /// User crop as "x0,y0,x1,y1".  Empty -> no override.
    std::string crop;

    /// Text overlay to bake into the image ("tarja").  Empty -> none.
    std::string textOverlay;

    /// Arbitrary "User_*" metadata.  Keys are passed verbatim to the
    /// server, prefixed with "User_" if not already.  Values are
    /// URL-escaped automatically by the SDK.
    std::map<std::string, std::string> userMetadata;

    /// Optional Cenario value override (1-9).  -1 leaves unchanged.
    /// Kept separate from @ref scenario for clarity with the legacy
    /// `c=<single digit>` form expected by the daemon.
    int cenario = -1;
};
...
/// Decoded image returned by /api/lastframe.cgi or /api/snapshot.cgi.
/// For multipart responses (multi-exposure without mosaic) the parser
/// yields one CgiImage per part.
struct CgiImage {
    std::string                        mimeType;     // "image/jpeg", "image/png"
    std::vector<uint8_t>               data;         // encoded image bytes
    std::map<std::string, std::string> headers;      // per-part headers
};

/// Raw HTTP response used by ItscamCgiClient::httpGetRaw escape hatch.
struct CgiResponse {
    int                                status = 0;
...
//=========================================================================
// CGI MJPEG / trigger streaming frame
//=========================================================================

/// Single frame delivered through the MJPEG or trigger stream.
struct CgiStreamFrame {
    uint64_t                           sequence = 0; // monotonic counter
    std::string                        mimeType;     // usually "image/jpeg"
    std::vector<uint8_t>               data;         // JPEG bytes
    std::map<std::string, std::string> headers;      // multipart part headers
};

using CgiStreamCallback = std::function<void(const CgiStreamFrame&)>;
...
struct ScenarioCrop {
    ...
    ScenarioCrop() = default;
    ScenarioCrop(int x0, int y0, int x1, int y1)
