...
> **Referência completa de cada método** (assinatura, typed structs, overloads): veja a [referência Doxygen do C++](/api-ref/cpp/classitscam_1_1ItscamRestClient.html). Esta página foca em conceitos (typed surface vs escape hatch, serialização parcial, error handling) e exemplos.

O REST client expõe duas surfaces que coexistem:

* **Typed helpers** (preferido). `getOcrConfig()` devolve `Result<OcrConfig>`, `setOcrConfig(OcrConfig)` aceita e devolve a struct tipada. Os types vivem em [`src/core/itscam_rest_types.h`](../../src/core/itscam_rest_types.h) e são **gerados automaticamente** a partir do documento OpenAPI da câmera. Veja [`docs/codegen.md`](../codegen.md) para o workflow de refresh / regeneration.
...
O update de software usa as rotas autenticadas do webapp backend. Faça
`login()` ou `setAuthToken()` antes de chamar estes métodos.
...
O arquivo SWU é lido pelo core C++ e enviado como `multipart/form-data`, sem
copiar o archive inteiro para memory das wrappers. O core também conecta no
websocket `/api/swupdate` antes do upload e acompanha mensagens `status`,
`message` e `step` até `SUCCESS`, `FAILURE`, timeout ou cancelamento.

Para operação non-blocking, use `startSoftwareUpdate()`, registre callback com
`setCallback()`, consulte `status()` quando quiser e finalize com `wait()` ou
`cancel()`.

`ws://` é suportado pelo transporte interno mínimo. Para bases configuradas com
`https`, a operação retorna `ConnectionFailed` até o backend vendorizado com
libwebsockets ser adicionado para `wss://`.
...
> **Quando recorrer ao escape hatch:** schema gaps (por exemplo, um endpoint que ainda não foi promovido para um typed wrapper), fields mais novos que o snapshot do SDK, payloads não tipados para endpoints sem typed helper ou chamadas diagnósticas one-off. Caso contrário, a typed surface é preferível por compile-time safety, IDE completion e paridade entre language bindings.

## Partial updates: typed setters com serialização parcial

Setters tipados (e.g. `updateProfileById`, `SetOcrConfigAsync`, `set_ocr_config`, `SetOcrConfig`) usam **serialização parcial**: fields unset/null/nil/nullopt são automaticamente omitidos do body PUT. Construa um struct tipado com apenas os fields que você quer mudar e passe diretamente ao setter -- o daemon faz merge dos campos enviados sobre a configuration existente.

```csharp
// C# -- typed partial update (preferencial)
...
### Alternativa genérica: patchJson

Os métodos genéricos `patchJson` / `PatchJsonAsync` / `patch_json` / `PatchJSON` permanecem disponíveis para payloads não tipados, endpoints sem typed helper ou quando construir o patch a partir de JSON cru é mais conveniente:
...
Os exemplos C# `MjpegGrabberExample` e `SoftwareTriggerSnapshotExample` usam `GetProfilesAsync()` para encontrar profile ids (read-only) e setters tipados ou `PatchJsonAsync()` para writes. O [`ProfileConfigExample`](../../src/wrappers/csharp/examples/ProfileConfigExample/) mostra um cenário completo de equipment configuration: configura os profiles `Diurno` (1 exposure) e `Noturno` (2 exposures com flash 100% / 5%) com trigger contínuo via `UpdateProfileByIdAsync`, configura o MJPEG stream com `SetStreamConfigAsync` + um `PatchJsonAsync` para o flag `useTriggerFrames` (fora do typed schema) e consome o stream resultante com `ItscamCgiClient.MjpegFrame`.
