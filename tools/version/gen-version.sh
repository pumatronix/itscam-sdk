...
def safe_write(path: Path, text: str) -> None:
    ...
    build left the file owned by root.  Creates parent directories so
    OUT_HEADER_DIR paths that don't exist yet work without manual setup."""
...
def reuse_locked_version() -> bool:
    ...
    _out_header_dir = os.environ.get("OUT_HEADER_DIR")
    target_header = (
        Path(_out_header_dir) / "itscam_sdk_version.h"
        if _out_header_dir
        else root / "src/core/itscam_sdk_version.h"
    )
    ...
    target_header.write_text(
        ...
        f"#pragma once\n\n"
        f'#define ITSCAM_SDK_VERSION_MAJOR {major}\n'
        f'#define ITSCAM_SDK_VERSION_MINOR {minor}\n'
        f'#define ITSCAM_SDK_VERSION_PATCH {patch}\n'
        f'#define ITSCAM_SDK_VERSION_STRING "{lib_ver}"\n'
        f'#define ITSCAM_SDK_VERSION_FULL "{data["versionFull"]}"\n'
        f'#define ITSCAM_SDK_GIT_SHA "{data["gitSha"]}"\n'
        f'#define ITSCAM_SDK_GIT_SHA_SHORT "{data["gitShaShort"]}"\n'
        f'#define ITSCAM_SDK_BUILD_DATE "{data["buildDate"]}"\n',
        encoding="utf-8",
    )
    ...
    safe_write(out_mk,
        "\n".join([
            ...
            f"SDK_VERSION_MAJOR := {major}",
            f"SDK_VERSION_MINOR := {minor}",
            f"SDK_VERSION_PATCH := {patch}",
            f"SDK_LIB_VERSION := {lib_ver}",
            f"SDK_VERSION := {mk_escape(data['version'])}",
            f"SDK_VERSION_FULL := {mk_escape(data['versionFull'])}",
            f"SDK_MAVEN_VERSION := {mk_escape(data.get('mavenVersion', lib_ver))}",
            f"SDK_GIT_SHA := {mk_escape(data['gitSha'])}",
            f"SDK_GIT_SHA_SHORT := {mk_escape(data['gitShaShort'])}",
            f"SDK_BUILD_DATE := {mk_escape(data['buildDate'])}",
            "",
        ]))
...
def compute() -> dict[str, str | int]:
    ...
    if git_available():
        ...
        dirty = False
        commits_since = 0
        # No git available (Yocto/Buildroot sandbox, release tarball, etc.).
        # Read metadata from VERSION.json (committed at release time) so the
        # generated header carries the real SHA and version rather than
        # falling back to the existing header or hard-coded 0.0.0.
        _version_json = root / "VERSION.json"
        if _version_json.exists():
            try:
                _vdata = json.loads(_version_json.read_text(encoding="utf-8"))
                git_sha = _vdata.get("gitSha", "0" * 40)
                git_sha_short = _vdata.get("gitShaShort", "0000000")
                base_version = _vdata.get("libVersion", fallback_from_header())
            except Exception:
                git_sha = "unknown"
...
info = compute()
header_only = os.environ.get("ITSCAM_VERSION_HEADER_ONLY") == "1"

_out_header_dir = os.environ.get("OUT_HEADER_DIR")
core_header = Path(_out_header_dir) / "itscam_sdk_version.h" if _out_header_dir else root / "src/core/itscam_sdk_version.h"
...
if header_only:
    print(
        ...
        f"(lib {info['lib_version']}, {info['git_sha_short']}, {info['build_date_short']}) [header-only]"
    )
    # Write sdk-version.mk even in header-only mode so that Makefile builds
    # that set OUT_HEADER_DIR still get the correct SDK_LIB_VERSION / SONAME.
    safe_write(out_mk,
        "\n".join(
            [
                "# Auto-generated by tools/version/gen-version.sh -- do not edit.",
