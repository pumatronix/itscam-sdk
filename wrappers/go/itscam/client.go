...
// The result will be delivered via the snapshot callback if set.
func (c *Client) RequestSnapshot(timeout time.Duration) error {
	...
	if result != C.ITSCAM_OK {
		return errorFromCode(int(result))
	}

	// Free the result array since we expect results via callback
	if resultArray != nil {
		C.ITSCAM_CaptureResultArray_destroy(resultArray)
	}
